...
def main():
    ...
    for line in open("WES_REF_Tier1_maf.txt"):
          record = line.strip().split(',')
          MAFs[record[0].strip()] = record[1].strip()
    ...
    for line in open(args.infile2): 
            ...
            if sid in gtsnps.keys() and  sid in MAFs.keys(): 
                MAF = float(MAFs[sid])
                gt = rows[9].split(":")
                dp = int(gt[2])
                ...
                print(str(sid).strip()+','+str(alt), file=acfile)
